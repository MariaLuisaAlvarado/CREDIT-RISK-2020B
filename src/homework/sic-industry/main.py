...
class Main:
    ...
    @staticmethod
    def foresito(dictionary: dict, target: str, exact: bool = True):
        list1 = []
        for element in dictionary.items():
            title = element[0]
            children = element[1]
            title_sw = StringWrapper(value=title,
                                    case_sensitive=False)
            valid = title_sw.boolean_search(pattern=target,
                                            exact=exact,
                                            threshold=INDUSTRY_SEARCH_DEFAULT_THRESHOLD)
            if valid is True:
                list1.append(valid)
            if isinstance(children, list):
                self.foresito(children)
        return list1
